const language = {
    ...
    error1: "プロジェクトに入力したボットのトークンが間違っているか、ボットのINTENTSがオフになっています!",
    error2: "ボットのトークンをtoken.jsまたはプロジェクト内の.envファイルに設定してください。!",
    loadclientevent: "クライアントイベントの読み込みが完了",
    embed1: "このコマンドを使用するには、このサーバーに <@&{djRole}>(DJ) ロールが設定されている必要があります。このロールを持たないユーザは {cmdMAP} を使用できません。",
    ...
    error4: "⚠️ 警告: mongodbのurlを書いてないようです。config.jsファイルに有効なmongodbのurlを書かないと、ボットを使用することができません。",
    msg13: `🎵 現在再生中： **{track?.title}** -> チャンネル: **{queue?.connection.channel.name}** 🎧`,
    msg14: "❌ キューが空です。",
    msg15: "❌ 自分のチャンネルに誰もいなくなったので、切断しました。",
    msg16: "❌ ボイスチャンネルに接続できませんでした。誰かに切断させられたようです。悲しいです…😔",
    msg17: "❌ 前のトラックはありません！",
    ...
    msg32: "完全無料かつ高度なインターフェイスで、あなたのdiscordサーバーで楽曲を聴くときが来たのです。多くのプラットフォームでの音楽再生をサポートする音楽ボットで、あなたのサーバーを特別な雰囲気にします。あなただけの音楽ボットを作りましょう！https://github.com/umutxyp/MusicBot",
    msg33: "ボットのコマンド",
    ...
    msg118: "一定時間経過したため、プレイリストの表示を終了しました。再度使用するには `/playlist list {プレイリスト名}` を実行してください。",
    msg119: "これらのプレイリストを聴くには、**/play playlist <プレイリスト名>**コマンドを実行してください。\n楽曲をリストで見るには、**/playlist list <プレイリスト名>**を実行してください。",
    msg120: "テキストチャンネルを指定してください。",
    msg121: "<#{channel}> をコマンド使用チャンネルに追加しました。これにより、それ以外のチャンネルではコマンドが使用できなくなりました。",
    ...
    msg126: "❌ このサーバーでコマンド可能なチャンネルの一覧です： {channel_filter}",
    msg127: "コマンドは定義されていません。",
    error7: "このコマンドは後でもう一度試してください。ボット開発者に報告されたバグの可能性があります。",
    msg128: "楽曲再生中にミュートに設定されたため、楽曲の再生を停止しました。再生を再開するには、ミュートを解除してください。",
    msg129: "再生",
