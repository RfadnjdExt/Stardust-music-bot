...
module.exports = {
    ...
    run: async (_, interaction) => {
        ...
        const urlDict = {
            ...
            "star-rail": "https://webstatic.mihoyo.com/app/community-shop/index.html?#!/evt_info/azaocr803",
            hi3: "https://webstatic.mihoyo.com/bh3/event/e20211029sign/index.html"
        };
        const url = urlDict[game];

        if (!url) {
            return await interaction.reply(`Maaf, saya tidak mengenali permainan ${game}.`);
        }
        ...
        const rewards = html.match(/(?<=<span class="info-title ellipses">).+?(?=<\/span>)/g);
        const messageEmbed = new MessageEmbed();
        messageEmbed
            .setColor("#00ff00")
            .setTitle(`Berhasil tanda tangan harian ${game}!`)
            .setDescription(`Kamu berhasil tanda tangan harian ${game}. Berikut adalah hadiah yang kamu terima:`)
            .addFields(
                rewards.map(reward => {
